...
```{r eval=FALSE}
> myString <- "Good things comes in threes
+
```
...
This error may be due to a spelling error, so check the spelling of the variable name. It is also commonly encountered if you return to an old R script and try to run just a part of it - if the variable is created on an earlier line which hasn't been run, R won't find it because it hasn't been created yet.
...
This message is printed when you try to create a data frame with different number of rows for different columns, like in this example\index{error message!\texttt{arguments imply differing number of rows}}, where `a` has 3 rows and `b` has 4:
...
If you really need to create an object with different number of rows for different columns, create a `list` instead:
...
This error message is similar the the previous one, and appears when you try to apply a mathematical function, like `log` or `exp` to non-numerical variables\index{error message!\texttt{non-numeric argument to mathematical function}}:
...
This warning is thrown when you the condition in a conditional statement is a vector rather than a single value\index{warning message!\texttt{the condition has length > 1 and only the first element will be used}}. Here is an example:
...
This warning message (which perhaps should be a error message rather than a warning) occurs when you try to install a package\index{warning message!\texttt{package is not available}} that isn't available for the version of R that you are using.
