...
The `parallel` package contains parallelised version of the `apply` family of functions, with names like `parApply`, `parLapply`, and `mclapply`\index{\texttt{parApply}}\index{\texttt{parLapply}}\index{\texttt{mclapply}}\index{\texttt{makeCluster}}\index{\texttt{stopCluster}}. Which of these that you should use depends in part on your operating system, as different operating systems handle multicore computations differently. Here is the first example from Section \@ref(purrr), run in parallel with 3 workers:
...
### Integration with C++
C++ is commonly used to speed up functions, for instance involving loops that can't be vectorised or parallelised due to dependences between different iterations. The `Rcpp`\index{\texttt{Rcpp}} package allows you to easily call C++ functions from R, as well as calling R functions from C++. See `vignette("Rcpp-introduction", "Rcpp")` (Eddelbuettel & Balamuta, 2018) for details.
